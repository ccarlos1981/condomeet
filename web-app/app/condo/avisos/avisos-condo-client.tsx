    ...
    <div className="max-w-3xl">
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-1">
          <Bell size={22} className="text-[#FC5931]" />
      ...
      <section className="mb-8">
        <div className="flex items-center gap-2 mb-3">
          <Bell size={16} className="text-[#FC5931]" />
          <h2 className="text-base font-semibold text-gray-800">Não lidos</h2>
          {naoLidos.length > 0 && (
            <span className="bg-[#FC5931] text-white text-xs font-bold px-2 py-0.5 rounded-full">
              {naoLidos.length}
            </span>
          )}
        </div>
        ...
        {naoLidos.length === 0 ? (
          ...
          <div className="space-y-3">
            {naoLidos.map(aviso => (
              <div
                key={aviso.id}
                className="bg-white rounded-2xl border-l-4 border-l-[#FC5931] border-y border-r border-gray-100 shadow-sm overflow-hidden"
              >
                <button
