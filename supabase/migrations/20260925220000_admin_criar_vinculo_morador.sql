-- ==============================================================================
-- Migration: 20260925220000_admin_criar_vinculo_morador.sql
-- Module: Base Cadastral Unificada (Gate 3C.7)
-- Purpose: Canonical transactional RPC for restoring/creating resident occupancy
--          link (Novo Vínculo) without reopening or mutating previous historical
--          periods, adhering strictly to: RETORNO = NOVO VÍNCULO / NOVO PERÍODO (INSERT).
-- ==============================================================================
...
BEGIN
    ...
    -- Bloqueio administrativo disciplinar ativo (Seção 6)
    ...
    -- Não permitir perfis pendentes ou reprovados (Seção 5)
    ...
    -- Validar elegibilidade de status: deve ser inativo ou proprietário não morador (Seções 5 e 17)
    ...
    -- 5. VALIDAÇÃO DE VÍNCULO ATIVO EXISTENTE (Seção 13 - UMA RESIDÊNCIA ATIVA NO FLUXO DE RETORNO)
    IF EXISTS (
    ...
    -- 6. VALIDAÇÃO DA UNIDADE RESIDENCIAL (Seção 8)
    ...
    -- 7. VALIDAÇÃO DO TIPO DE MORADOR (Seção 9)
    ...
    -- 8. VALIDAÇÃO DE SOBREPOSIÇÃO TEMPORAL NA MESMA UNIDADE (Seção 10 e 11)
    IF EXISTS (
        SELECT 1 FROM public.unidade_perfil
        ...
        RAISE EXCEPTION 'A data de entrada não pode ser anterior à data de saída do período anterior nesta mesma unidade.';
    END IF;

    -- 9. INSERT DO NOVO PERÍODO (Seção 15 - RETORNO = NOVO VÍNCULO / NOVO PERÍODO)
    ...
    -- 10. UPDATE DO PERFIL DO MORADOR (Seções 16 e 17)
    -- Preserva integralmente: papel_sistema, email, telefone, whatsapp, auth id
    ...
    -- 11. REGISTRO ATÔMICO DE AUDITORIA (Seção 19)
    ...
    -- 12. RETORNO ESTRUTURADO SANITIZADO (Seção 21)
...
-- Restrição de privilégios de execução (Seção 22)
...
IS 'Cria um novo período/vínculo residencial ativo para morador inativo preservando períodos históricos anteriores (Gate 3C.7).';
