-- ============================================================
-- Migration: 20260309i — pg_cron: check contratos vencimento
...
SELECT cron.unschedule('check-contratos-vencimento')
WHERE EXISTS (
  SELECT 1 FROM cron.job WHERE jobname = 'check-contratos-vencimento'
);

-- Agenda o job diário
SELECT cron.schedule(
  'check-contratos-vencimento',    -- nome do job
  ...
  $$
    SELECT net.http_post(
      url     := COALESCE(
                 ...
                 ) || '/functions/v1/contratos-vencimento-check',
