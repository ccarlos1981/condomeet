...
async function callGemini(
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    ...
    if (res.ok) {
      ...
      const responsePart = parts.filter((p: Record<string, unknown>) =>
        !p.thought
      ).pop();
    ...
    if (res.status === 429 && attempt < MAX_RETRIES) {
      ...
      console.warn(
        `[Gemini] Rate limited (429), retrying in ${delayMs}ms (attempt ${
          attempt + 1
        }/${MAX_RETRIES})`,
      );
    ...
    console.error(
      `[Gemini] Error ${res.status} URL=${url.split("?")[0]}: ${
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (incoming.fromMe) {
      console.log(
        `[Webhook] Ignoring outgoing message (fromMe=true) to: ${incoming.phone}`,
      );
      return jsonResponse({
        skipped: true,
        reason: "Outgoing message (fromMe) ignored",
      });
    }
    ...
    if (shouldIgnoreMessage(incoming.text, incoming.messageType)) {
      console.log(
        `[Webhook] Ignoring message: type=${incoming.messageType}, text="${
          incoming.text.substring(0, 50)
        }"`,
      );
      return jsonResponse({
        skipped: true,
        reason: "Non-text or emoji ignored",
      });
    }

    // Deduplication: prevent processing same message twice
    const dedupKey = `${incoming.phone}:${
      incoming.messageId || incoming.text.substring(0, 50)
    }`;
    ...
    console.log(
      `[Webhook] From: ${incoming.phone}, Type: ${incoming.messageType}, Text: "${
        incoming.text.substring(0, 100)
      }"`,
    );
    ...
    let pending: { last_arrival: string } | null = null;
    try {
    ...
    if (pending && pending.last_arrival > arrivalTs) {
      console.log(
        `[Debounce] Newer message detected for ${incoming.phone}, skipping this one`,
      );
      return jsonResponse({
        skipped: true,
        reason: "Debounce: newer message arrived",
      });
    }
    // ─────────────────────────────────────────────────────────────────────

    const isAdmin = ADMIN_PHONES.some((p) =>
      incoming.phone.replace(/\D/g, "").endsWith(p.replace(/\D/g, ""))
    );
    if (isAdmin) {
      const cmd = incoming.text.trim().toUpperCase().normalize("NFD").replace(
        /[\u0300-\u036f]/g,
        "",
      );
      ...
      if (cmd === "STATUS") {
        const { data: cfg } = await supabase.from("bot_config").select(
          "ativo, desativado_em, reativado_em",
        ).eq("id", 1).single();
        const statusMsg = cfg?.ativo
          ? `🟢 Bot está ATIVO.${
            cfg.reativado_em
              ? ` Reativado em: ${
                new Date(cfg.reativado_em).toLocaleString("pt-BR")
              }`
              : ""
          }`
          : `🔴 Bot está DESATIVADO.${
    ...
    const phoneVariants = [
      incoming.phone,
      incoming.phone.startsWith("55")
        ? incoming.phone.substring(2)
        : `55${incoming.phone}`,
    ];
    ...
    for (const variant of phoneVariants) {
      const { data } = await supabase
        .from("perfil")
        .select(
          "id, nome_completo, bloco_txt, apto_txt, condominio_id, tipo_morador, papel_sistema, status_aprovacao, whatsapp, notificacoes_whatsapp",
        )
        .eq("whatsapp", variant)
    ...
    if (!perfil) {
      const phone = incoming.phone.startsWith("55")
        ? incoming.phone
        : `55${incoming.phone}`;
    ...
    if (!perfil) {
      console.log(
        `[Webhook] No approved profile found for phone: ${incoming.phone}`,
      );
      ...
      if (!count || count === 0) {
        ...
        console.log(
          `[Webhook] First message from unknown number ${incoming.phone} — ignoring silently`,
        );
        ...
        return jsonResponse({
          skipped: true,
          reason: "Unknown number — first attempt, silent ignore",
        });
      }
    ...
    if (perfil.notificacoes_whatsapp === false) {
      console.log(
        `[Webhook] User ${perfil.id} has opted out of WhatsApp notifications`,
      );
      return jsonResponse({ skipped: true, reason: "User opted out" });
    }

    console.log(
      `[Webhook] Identified: ${perfil.nome_completo} (${perfil.bloco_txt}/${perfil.apto_txt})`,
    );
    ...
    const geminiHistory: GeminiMessage[] = (historyRows || []).map((
      row: any,
    ) => ({
    ...
    try {
      console.log(
        `[Gemini] Calling with ${geminiHistory.length} history messages`,
      );
      ...
      const errMsg = geminiErr instanceof Error
        ? geminiErr.message
        : String(geminiErr);
    ...
    if (geminiResponse.actions.length > 0) {
      console.log(
        `[Actions] Executing ${geminiResponse.actions.length} action(s):`,
        geminiResponse.actions.map((a) => a.type),
      );
    ...
    return jsonResponse({
      ...
      morador:
        `${perfil.nome_completo} (${perfil.bloco_txt}/${perfil.apto_txt})`,
    });
  } catch (error: unknown) {
