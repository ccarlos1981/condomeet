...
// Monta título e corpo da notificação conforme o tipo de evento
...
serve(async (req) => {
  ...
  try {
    ...
    // Busca FCM tokens de todos os moradores aprovados do condomínio
    ...
    const { title, body } = buildNotification(titulo ?? 'Documento do condomínio', tipo_evento ?? 'novo_documento')
    ...
    for (const token of tokens) {
      const message = {
        message: {
          ...
          android: {
            priority: 'high',
            notification: { channel_id: 'documentos' },
          },
          apns: {
