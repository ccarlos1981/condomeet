    ...
    if (action === "pending_approval") {
      ...
      if (hasSpecificUser && resident) {
        if (BOTCONVERSA_API_KEY && resident.notificacoes_whatsapp !== false && (resident.botconversa_id || (resident.whatsapp as string)?.trim())) {
          ...
          const result = await smartSend(BOTCONVERSA_API_KEY, resident.botconversa_id as string, resident.whatsapp as string, "text", msgResident, undefined, supabase, resident.id as string, MessageType.RESERVATION, "reserva-notify")
        ...
        for (const r of unitResidents) {
          if (BOTCONVERSA_API_KEY && r.notificacoes_whatsapp !== false && (r.botconversa_id || (r.whatsapp as string)?.trim())) {
            const result = await smartSend(BOTCONVERSA_API_KEY, r.botconversa_id as string, r.whatsapp as string, "text", msgUnit, undefined, supabase, r.id as string, MessageType.RESERVATION, "reserva-notify")
      ...
      if (resident?.notificacoes_whatsapp !== false && BOTCONVERSA_API_KEY && (resident?.botconversa_id || resident?.whatsapp)) {
        ...
        const result = await smartSend(BOTCONVERSA_API_KEY, resident.botconversa_id, resident.whatsapp, "text", msg, firstName, supabase, resident.id, MessageType.RESERVATION, "reserva-notify")
        results.push(`WhatsApp morador: ${result.success ? "✅" : "❌"}`)
      }
