...
class _Document {
  ...
  factory _Document.fromMap(Map<String, dynamic> m) {
    return _Document(
      id: m['id'] as String,
  ...
  bool get isExpired {
    if (semValidade || dataValidade == null) return false;
...
class _DocumentsScreenState extends State<DocumentsScreen> {
  ...
  bool _loading = true;
  String _search = '';
  String? _selectedTipo; // 'obrigatorio', 'manutencao', 'outros' ou null
  ...
  Future<void> _load() async {
    ...
    try {
      final res = await Supabase.instance.client
          .from('documentos')
          .select('id, titulo, tipo, categoria, sem_validade, arquivo_url, arquivo_nome, data_validade, created_at')
  ...
  List<_Document> get _filtered {
    ...
    return _allDocs.where((d) {
      ...
      final normTipo = normalizeTipoDocumento(d.tipo);
      final matchesTipo = _selectedTipo == null || normTipo == _selectedTipo;
      ...
      child: SingleChildScrollView(
        ...
        child: Row(
          children: [
            ...
            _filterChip('Obrigatórios', 'obrigatorio'),
            _filterChip('Manutenções', 'manutencao'),
            _filterChip('Outros', 'outros'),
          ],
        ),
      ),
    );
  ...
  Widget _filterChip(String label, String? value) {
    final selected = _selectedTipo == value;
    return Padding(
      ...
      child: ChoiceChip(
        ...
        onSelected: (_) => setState(() => _selectedTipo = value),
      ),
    );
  }
                    ...
                    Wrap(
                      ...
                      children: [
                        ...
                        if (doc.categoria != null && doc.categoria!.isNotEmpty)
                          Text(doc.categoria!, style: TextStyle(fontSize: 11, color: Colors.grey.shade700, fontWeight: FontWeight.w500)),
                        // Validade
                        if (doc.semValidade)
                          Text('• Permanente', style: TextStyle(fontSize: 10, color: Colors.grey.shade500))
